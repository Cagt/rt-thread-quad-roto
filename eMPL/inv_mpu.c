...
//}
#define log_i 	rt_kprintf	//��ӡ��Ϣ
#define log_e  	rt_kprintf	//��ӡ��Ϣ
...
int mpu6050_init(void)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data) == RT_ERROR){
        rt_kprintf("error");
        return -1;
    }
    delay_ms(100);
    ...
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data) == RT_ERROR)
        return -1;
    ...
    if (rev) {
        ...
        if (i2c_read(st.hw->addr, st.reg->prod_id, 1, data)  == RT_ERROR)
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
